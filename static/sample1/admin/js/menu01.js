...
//왼쪽 체크 라디오 버튼 가져오기
    ...
    confirmButton.addEventListener('click', async function() {
        ...
        const response = await fetch('/sample1/admin/upload_media_data_img/', {
            method: 'POST',
            headers: {
...
//체크 라디오 버튼 가져오기
    ...
    async function data_list(page_number){
        ...
        // 요청 보내기
        const response = await fetch("/sample1/admin/data_list_image/", {
            method: "POST",
            headers: {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        modal.querySelector('.btn-primary').addEventListener('click', async function() {
            ...
            // 여기에 실제 삭제 처리 로직
            // 요청 보내기
            const response = await fetch("/sample1/admin/data_list_image_delete/", {
                method: "POST",
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded',
                    'X-CSRFToken': csrfToken

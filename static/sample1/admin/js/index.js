...
//카테고리 불러오기
        ...
        const response = await fetch("/sample1/admin/category_list/", {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
            console.log("/sample1/admin/category_list/")
            /* 데이터 가져오기 */
            ...
            for(var i=0; i<category_line_container_list.length; i++) {
                ...
                //카테고리 URL 값을 넣는다
                //getData 값을 넣는다
                // category_line_container_list[i].querySelector('.category_url').value = "https://fc3.kr/sample1/?cn=" + categoryListFromServer[i] + "&v1=&v2=&v3=&v4=&v5=&v6=&v7=&v8=&v9=&v10=";
                category_line_container_list[i].querySelector('.category_url').value = "http://52.78.250.255/kms/";
...
//카테고리에서 수정 버튼 클릭시
            ...
            document.addEventListener('click', async function(event) {
              ...
              if (event.target.classList.contains('category_edit')) {
                ...
                /* 응답 요청 await 함수는 async 내에서만 사용 가능*/
                const response = await fetch("/sample1/admin/category_data/", {
                    method: "POST",
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded',
                        'X-CSRFToken': csrfToken
                ...
                if (response.ok) {
                    ...
                    } else if(data.category_GPT == "gpt-4o"){
                    ...
                    } else if(data.category_data_only == "notonly"){
            ...
            document.addEventListener('click', async function(event) {
                if (event.target.classList.contains('btn-outline-danger')) { // '삭제' 버튼 클릭 시
                    ...
                    document.getElementById('btn_confirm_category_delete').addEventListener('click', async function() {
                        ...
                        /* 응답 요청 await 함수는 async 내에서만 사용 가능*/
                        const response = await fetch("/sample1/admin/category_delete/", {
                            method: "POST",
                            headers: {
                                'Content-Type': 'application/x-www-form-urlencoded',
                                'X-CSRFToken': csrfToken
...
//특정 라디오 버튼 클릭시 이벤트
    ...
    document.getElementById('customRadio9').addEventListener('click', function () {
        form_data_text.style.display = 'none'//오직 내 데이터에만 의존하면 찾기 실패 메시지 보이기
        form_data_file.style.display = 'none'
        form_nodata.style.display = 'none'//다른 데이터도 가능하면 찾기 실패 메시지 보이지 않기
    });
...
//전송 버튼 눌렀을 때
    document.querySelector(".confirm_button").addEventListener("click", async(event) => {
        ...
        const radioBtn9 = document.getElementById('customRadio9');
        ...
        if(radioBtn9.checked){
            console.log('customRadio9 버튼이 체크되었습니다.');
            categoryDataOnly_input = "notonly";
        }
        ...
        /* 응답 요청 await 함수는 async 내에서만 사용 가능*/
        const response = await fetch("/sample1/admin/send_data/", {
            method: "POST",
            headers: {
                'X-CSRFToken': csrfToken
            },
